...
namespace MethodAssignment
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            optional.numOne = Convert.ToInt32(Console.ReadLine());
            ...
            if (user != "")
            {
                optional.numTwo = Convert.ToInt32(user);
                Console.WriteLine("Number one + (number two * 5) is: " + optional.timesFive(optional.numOne, optional.numTwo));
            }
            else
            {
                Console.WriteLine("Number one + (number two * 5) is: " + optional.timesFive(optional.numOne));
            }
            Console.ReadLine();
        }
    }
